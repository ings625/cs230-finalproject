...
def create_image_lists(image_dir, testing_percentage, validation_percentage, training_total_size):
  ...
  eval_labels = [str(int(f.split('/')[-1].split('_')[0])) for f in eval_filenames]
  eval_labels_set = set(eval_labels)
  # print(eval_labels_set)
  # eval_labels = [label_mappings[t] for t in eval_labels]

  test_labels = [str(int(f.split('/')[-1].split('_')[0])) for f in test_filenames]
  ...
  for t in train_labels_set:
    ...
    for ev in range(len(eval_labels)):
      if eval_labels[ev] == t:
        validation_images.append(eval_filenames[ev])

    for te in range(len(test_labels)):
      if test_labels[te] == t:
        testing_images.append(test_filenames[te])
...
def run_final_eval(sess, model_info, class_count, image_lists, jpeg_data_tensor,
  ...
  csvpath = "test_output_labels.csv"
